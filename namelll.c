...
/*int sortNames()
{
	...
	while (tempNode != '\0')
	{
		if (strcmp(tempNode -> name, tempNode -> nextNode -> name) > 0)
		{
...
/*int deleteName()
{
	...
	while(strcmp(tempNode -> name, nameToBeDeleted) != 0)
	{
...
int deleteName(struct node *deleteNode)
{
	struct node **tempNode;
	tempNode = &start;
	while (*tempNode != deleteNode)
	{
		tempNode = &(*tempNode) -> nextNode;
	}
